...
function updateTime() {
  curTime += 60.0 / 4 / parseInt($('.bpm-input').val(), 10)
  noteCount++
}
...
$('.tap-btn').click(function () {
  const d = new Date()
  const temp = parseInt(d.getTime(), 10)
...
$('.ts-top, .ts-bottom').on('change', function () {
  ...
  for (let i = 0; i < parseInt($('.ts-top').val(), 10); i++) {
