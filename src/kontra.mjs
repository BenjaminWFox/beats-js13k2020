...
class Animation {
  constructor({ spriteSheet, frames, frameRate, loop = true }) {
    ...
    const { width, height, margin = 0 } = spriteSheet.frame
...
function getName(url) {
  const name = url.replace(`.${ getExtension(url)}`, '')
...
function loadAudio(url) {
  return new Promise((resolve, reject) => {
    const _url = url; let audioEl; let canPlay; let resolvedUrl; let
      fullUrl
    ...
    url = [].concat(url)
      .reduce(
        (playableSource, source) => playableSource || (canPlay[getExtension(source)]
          ? source
          : null)
        , 0,
...
function load(...urls) {
  ...
  return Promise.all(urls.map((asset) => {
    ...
    const extension = getExtension([].concat(asset)[0])
...
function seedRand(str) {
  ...
  const rand = () => (2 ** 31 - 1 & (seed = Math.imul(48271, seed))) / 2 ** 31
...
function collides(obj1, obj2) {
  if (obj1.rotation || obj2.rotation) {
    return null
  }
...
function addToDom(node, canvas) {
  ...
  if (container) {
    const target = container.querySelector('[data-kontra]:last-of-type') || canvas
...
class GameObject extends Updatable {
  ...
  _uw() {
    ...
    const { x, y } = rotatePoint({ x: this.x, y: this.y }, _wr)
...
function circleRectCollision(object, pointer) {
  ...
  const dx = pointer.x - Math.max(x, Math.min(pointer.x, x + width))
  const dy = pointer.y - Math.max(y, Math.min(pointer.y, y + height))
...
function getCurrentObject(pointer) {
  ...
  const renderedObjects = pointer._lf.length ?
    pointer._lf :
    pointer._cf

  for (let i = renderedObjects.length - 1; i >= 0; i--) {
    const object = renderedObjects[i]
    const collides = object.collidesWithPointer ?
...
function getCanvasOffset(pointer) {
  ...
  // canvas
  const { canvas, _s } = pointer
  const rect = canvas.getBoundingClientRect()

  // @see https://stackoverflow.com/a/53405390/2124254
  const transform = _s.transform !== 'none'
  ...
  const borderWidth = (getPropValue(_s, 'border-left-width') + getPropValue(_s, 'border-right-width')) * transformScaleX
  const borderHeight = (getPropValue(_s, 'border-top-width') + getPropValue(_s, 'border-bottom-width')) * transformScaleY

  const paddingWidth = (getPropValue(_s, 'padding-left') + getPropValue(_s, 'padding-right')) * transformScaleX
  const paddingHeight = (getPropValue(_s, 'padding-top') + getPropValue(_s, 'padding-bottom')) * transformScaleY
...
function pointerDownHandler(evt) {
  ...
  const button = evt.button !== undefined ? buttonMap[evt.button] : 'left'
...
function blurEventHandler(evt) {
  const pointer = pointers.get(evt.target)
...
function pointerHandler(evt, eventName) {
  ...
  const isTouchEvent = ['touchstart', 'touchmove', 'touchend'].indexOf(evt.type) !== -1
  ...
  if (isTouchEvent) {
    ...
    for (var i = evt.changedTouches.length; i--;) {
      ...
      const clientX = evt.changedTouches[i].clientX
      const clientY = evt.changedTouches[i].clientY

      pointer.x = (clientX - offsetX) / scaleX
      pointer.y = (clientY - offsetY) / scaleY

      // Trigger events
      const object = getCurrentObject(pointer)
...
function untrack(...objects) {
  objects.map((object) => {
    ...
    const index = pointer._o.indexOf(object)
...
class Button extends factory$3.class {
  ...
  _p() {
    ...
    const width = this.textNode.width + this.padX * 2
    const height = this.textNode.height + this.padY * 2
...
function GameLoop({
  ...
  const step = 1 / fps
  const clearFn = clearCanvas ? clear : noop
  let last; let rAF; let now; let dt; let loop
...
class Grid extends factory$2.class {
  ...
  _p() {
    ...
    const colGap = [].concat(this.colGap)
    const rowGap = [].concat(this.rowGap)
    ...
    this._g.map((gridRow, row) => {
      ...
      gridRow.map((child, col) => {
        ...
        if (child && !rendered.includes(child)) {
          ...
          const justify = alignment[child.justifySelf || this.justify](this._rtl)
          const align = alignment[child.alignSelf || this.align]()
          ...
          if (justify === 0) {
            ...
            const sign = anchorX < 0.5 ? -1 : anchorX === 0.5 ? 0 : 1
          ...
          if (align === 0) {
            ...
            const sign = anchorY < 0.5 ? -1 : anchorY === 0.5 ? 0 : 1
...
function getMethod(methodName) {
  const methodTitle = methodName.substr(methodName.search(/[A-Z]/))
...
function registerPlugin(kontraObj, pluginObj) {
  const objectProto = kontraObj.prototype
  ...
  Object.getOwnPropertyNames(pluginObj).forEach((methodName) => {
    ...
    if (!objectProto[`_o${ method}`]) {
      ...
      objectProto[method] = function interceptedFn(...args) {
        ...
        const alteredArgs = this._bInc(this, method, ...args)

        const result = objectProto[`_o${ method}`].call(this, ...alteredArgs)
...
class Pool {
  ...
  get(properties = {}) {
    ...
    const obj = this.objects[this.size]
...
function getIndices(object, bounds) {
  ...
  const intersectsTopQuadrants = object.y < horizontalMidpoint
  const intersectsBottomQuadrants = object.y + object.height >= horizontalMidpoint
...
function parseFrames(consecutiveFrames) {
  ...
  const sequence = []
  const frames = consecutiveFrames.split('..')
...
class SpriteSheet {
  ...
  createAnimations(animations) {
    ...
    for (name in animations) {
      const { frames, frameRate, loop } = animations[name]
...
function TileEngine(properties) {
  ...
  const tileEngine = Object.assign({
    ...
    layerCollidesWith(name, object) {
      ...
      const row = getRow(y)
      const col = getCol(x)
      const endRow = getRow(y + height)
      const endCol = getCol(x + width)
    ...
    tileAtLayer(name, position) {
      const row = position.row || getRow(position.y)
      const col = position.col || getCol(position.x)
  ...
  tileEngine.tilesets.map((tileset) => {
    ...
    const url = (window.__k ? window.__k.dm.get(properties) : '') || window.location.href
    ...
    if (tileset.source) {
      ...
      const source = window.__k.d[window.__k.u(tileset.source, url)]
    ...
    if (`${tileset.image}` === tileset.image) {
      ...
      const image = window.__k.i[window.__k.u(tileset.image, url)]
  ...
  function renderLayer(layer, cX) {
    ...
    (layer.data || []).map((tile, index) => {
      ...
      const tilewidth = tileset.tilewidth || tileEngine.tilewidth
      const tileheight = tileset.tileheight || tileEngine.tileheight
      const margin = tileset.margin || 0

      const image = tileset.image

      const offset = tile - tileset.firstgid
      const cols = tileset.columns ||
        image.width / (tilewidth + margin) | 0

      const x = (index % tileEngine.width) * tilewidth
      const y = (index / tileEngine.width | 0) * tileheight
